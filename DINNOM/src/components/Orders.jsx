...
const Orders = () => {
            ...
            <div className="flex flex-col space-y-2">
              ...
              {canCancel ? (
                <button
                  ...
                  className="bg-gray-200 border border-gray-300 text-black px-4 py-2 rounded-lg text-sm hover:bg-red-100 w-full text-left transition-colors"
                >
                  Cancel Order
                </button>
              ) : (
                <button
                  disabled
                  className="bg-gray-100 border border-gray-200 text-gray-400 px-4 py-2 rounded-lg text-sm w-full text-left cursor-not-allowed"
                >
                  Cancel Order
                </button>
              )}
  ...
  const filteredOrders = orders.filter(order => {
    ...
    if (activeFilter === 'Orders') return order.status !== 'Cancelled';
    return true; // For 'Buy Again' etc., show all for now
  });
