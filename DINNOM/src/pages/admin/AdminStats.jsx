...
const AdminStats = () => {
  const [stats, setStats] = useState({ dailyStats: [], summary: { totalRevenue: 0, totalOrders: 0 }, categoryStats: [] });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        // Format date for charting
        const formattedData = data.dailyStats.map(item => ({
          ...item,
          date: new Date(item.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
        }));

        setStats({ dailyStats: formattedData, summary: data.summary, categoryStats: data.categoryStats || [] });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard title="Avg. Order Value" value={stats.summary.totalOrders > 0 ? `$${(stats.summary.totalRevenue / stats.summary.totalOrders).toFixed(2)}` : '$0.00'} />
      </div>

      {/* Charts */}
      <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        <h3 className="text-xl font-semibold mb-4 text-zinc-700">Sales & Orders Over Time (Last 90 Days)</h3>
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={stats.dailyStats} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" />
            <XAxis dataKey="date" stroke="#666" />
            <YAxis yAxisId="left" stroke="#8884d8" label={{ value: 'Sales ($)', angle: -90, position: 'insideLeft', fill: '#8884d8' }} />
